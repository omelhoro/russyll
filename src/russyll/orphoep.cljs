...
(defn dubcon
  ...
  ^{:doc "Cases like [грамм русский аккуратный коммунист рассада]"
    :user/comment "this is the best fn ever!"}
  [word]
  (-- word dubcon-pat (partial dubcon-alg word)))


(def fns [[hard-pron "hard-pron"] [unpal-e "unpal-e"] [cluster-assim "cluster-assim"] [end-assim "end-assim"] [reg-assim-repl "reg-assim-repl"] [reg-pal "reg-pal"] [ogo-ovo "ogo-ovo"] [chto-trans "chto-trans"] [dash-rep "dash-rep"] [dubcon "dubcon"]])
(def fns-mapped
  (map
   (fn [[fn-fn description]]
     (fn [word]
       (let [word-out (fn-fn word)] (do
                                      ; (println word-out " : " description)
                                      word-out))))
   fns))

(defn ^:export orpho-single [word]
  (reduce #(%2 %1) word fns-mapped))

...
(defn sm4-check-concl [c o]
  (let [last-let (str (last c))
    ...
    (do
      ;(println c o  last-let prelast value isreg)
...
(defn js-replace [[fm v c]]
  (do
      (++ [\~ (++ \| [v c]) \~]))
  )

(defn cluster-find [w]
  (-- w (second concluster-pat) js-replace))
...
(defn syll-s [models w]
  (let [
        phonw (reduce #(%2 %1) (.toLowerCase w) [orpho-single translit])
        clusters (divide-cluster (cluster-find (vow-clusters-sep phonw)))
        ]
    (map
      #(if
        (not= (.indexOf % "|") -1)
